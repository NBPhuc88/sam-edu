...
class UpdateSchoolClassRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'update_student_tuitions'   => ['nullable', 'boolean'],
        ];
    }
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator) {
            ...
            $currentStatus = $schoolClass ? (int) $schoolClass->status : Constant::CLASS_STATUS_ACTIVE;
            ...
            if (is_array($subjects)) {
                ...
                if ($currentStatus !== Constant::CLASS_STATUS_ACTIVE && $hasValidSubject) {
                    $existingSubjectIds = $schoolClass ? $schoolClass->classSubjects()->pluck('subject_id')->toArray() : [];
                    $newSubjectIds      = array_filter(array_map('intval', array_column($subjects, 'subject_id')));
                    ...
                    if ($existingSubjectIds !== $newSubjectIds) {
                        $validator->errors()->add(
                            'subjects',
                            'Chỉ lớp học ở trạng thái Đang hoạt động mới có thể thêm hoặc thay đổi môn học.'
                        );
                    }
                }
            }
