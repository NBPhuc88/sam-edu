...
export default function PlanEdit({ plan, errors = {} }: Props) {
    const [name, setName] = useState(plan.name || '');
    const [code, setCode] = useState(plan.code || '');
    const [price, setPrice] = useState<string>(String(plan.price ?? 0));
    const [yearlyPrice, setYearlyPrice] = useState<string>(
        plan.yearly_price !== null && plan.yearly_price !== undefined ? String(plan.yearly_price) : '',
    );
    const [durationDays, setDurationDays] = useState<string>(String(plan.duration_days ?? 30));
    const [maxStudents, setMaxStudents] = useState<string>(
        plan.max_students !== null && plan.max_students !== undefined ? String(plan.max_students) : '',
    );
    const [maxClasses, setMaxClasses] = useState<string>(
        plan.max_classes !== null && plan.max_classes !== undefined ? String(plan.max_classes) : '',
    );
    const [badgeText, setBadgeText] = useState(plan.badge_text || '');
    const [isFeatured, setIsFeatured] = useState(Boolean(plan.is_featured));
    ...
    return (
        <AppLayout title={`Chỉnh Sửa Gói: ${plan.name}`}>
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <div className="flex items-center gap-2 text-xs text-gray-500 font-medium mb-1">
                            ...
                            <span>/</span>
                            <span className="text-emerald-700 font-semibold">Chỉnh Sửa</span>
                        </div>
                        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                            <Edit2 className="h-7 w-7 text-amber-500" />
                            Chỉnh Sửa Gói Dịch Vụ: {plan.name}
                        </h1>
                        <p className="mt-1 text-sm text-gray-500">
                            Cập nhật thông tin giá, thời hạn, giới hạn tài nguyên và tính năng của gói cước.
                        </p>
                    </div>
                        ...
                        <Card className="p-6 bg-white border border-gray-200 shadow-xs space-y-5">
                            ...
                            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <Input
                                        type="text"
                                        placeholder="VD: standard, pro, trial_14d"
                                        value={code}
                                    ...
                                    <span className="text-[11px] text-gray-400 mt-1 block">
                                        Mã nhận diện duy nhất trong hệ thống và cổng thanh toán.
                                    </span>
                                </div>
                            ...
                            <div className="flex items-center gap-3 pt-2">
                                ...
                                />
                                <label htmlFor="is_featured_edit" className="text-sm font-medium text-gray-700 cursor-pointer">
