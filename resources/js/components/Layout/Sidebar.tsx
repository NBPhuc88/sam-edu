...
const TopNavLink: React.FC<{
    ...
    return (
        <Link
            href={item.path}
            onClick={handleClick}
            className={`group flex items-center gap-3 rounded-xl px-3.5 py-2.5 text-sm font-semibold transition-all ${active
        ...
        >
            {Icon && (
                <Icon
                    className={`h-4.5 w-4.5 shrink-0 transition-transform duration-150 ${active ? 'text-white' : 'text-gray-500 group-hover:text-gray-800'
                        }`}
                />
            )}
...
const SubNavLink: React.FC<{
    ...
    return (
        <Link
            href={item.path}
            onClick={handleClick}
            className={`group flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm transition-all ${active
                ? 'bg-emerald-50 text-emerald-800 font-bold border-l-2 border-emerald-600 pl-2.5'
                : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 font-medium'
                }`}
        >
            <span
                className={`h-1.5 w-1.5 rounded-full transition-colors shrink-0 ${active
                    ? 'bg-emerald-600'
    ...
    return (
        <div className="space-y-0.5">
            <button
                ...
                className={`group flex w-full items-center justify-between rounded-xl px-3.5 py-2.5 text-sm font-semibold transition-all ${groupActive
            ...
            >
                <div className="flex items-center gap-3">
                    {Icon && (
                        <Icon
                            className={`h-4.5 w-4.5 shrink-0 transition-colors ${groupActive ? 'text-emerald-600' : 'text-gray-500 group-hover:text-gray-800'
                                }`}
                        />
                    )}
                ...
                <ChevronDown
                    className={`h-4 w-4 shrink-0 transition-transform duration-200 ${expanded
...
export const Sidebar: React.FC<SidebarProps> = ({
    ...
    return (
        <>
            ...
            {/* Sidebar panel */}
            <aside
                className={`shrink-0 border-r border-gray-200 bg-white transition-all duration-300 ease-in-out ${open
                    ? 'w-64 opacity-100'
                    : 'pointer-events-none w-0 overflow-hidden border-r-0 border-transparent opacity-0'
                    } fixed inset-y-0 left-0 z-40 h-full shadow-xl md:sticky md:top-0 md:h-screen md:z-auto md:shadow-none ${open
                        ? 'translate-x-0'
